...
def move_snake():
    ...
    # On applique les nouvelles coordonnées au carré correspondant
    ...
    # Mouvement continu du serpent 

    if i != 1 and j != 1:
        déplacement = racine.after(k, move_snake)

    # Si le serpent touche le mur alors on a perdu et la partie s'arrete 
        
    if (x[0] < 25 or x[0] > WIDTH - 35) or (y[0] < 25 or y[0] > HEIGHT - 35):
        racine.after_cancel(déplacement)
        fin_du_jeu()

    # Si le serpent touche son propre corps alors on a perdu et la partie s'arrete 
...
" Fonction qui affiche le message (Game over) lorsqu'on a perdu  ainsi que le score du joueur"
...
"Fonction permettant de mettre pause"

def arreter(): 
    canvas.after_cancel(déplacement)
    pause.config(text = "PLAY", command=play)

"Fonction permettant de reprendre la partie aprés avoir mis sur pause"
...
# positionnement des widjets dans la fenetre du menu principal #
